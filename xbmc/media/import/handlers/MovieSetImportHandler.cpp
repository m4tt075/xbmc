...
bool CMovieSetImportHandler::AddImportedItem(const CMediaImport& import, CFileItem* item)
{
  ...
  {
    m_logger->error("failed to set details for added movie set \"{}\" imported from {}",
...
bool CMovieSetImportHandler::UpdateImportedItem(const CMediaImport& import, CFileItem* item)
{
  ...
  {
    m_logger->error("failed to set details for movie set \"{}\" imported from {}", item->GetLabel(),
                    import.GetPath());
    return false;
  }
                                           ...
                                           std::vector<CFileItemPtr>& items) const
{
  ...
  {
    m_logger->error("failed to get previously imported movie sets from {}", import.GetPath());
    return false;
  }
